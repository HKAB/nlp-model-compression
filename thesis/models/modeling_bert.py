...
class BertEncoder(nn.Module):
    ...
    def exit_inference_forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if exit_decision[0][1] > self.config.exit_port_threshold and entropy < self.config.entropy_threshold:
                break
        if output_hidden_states:
            all_hidden_states = all_hidden_states + (hidden_states,)
...
class BertModel(BertPreTrainedModel):
    ...
    def exit_inference_forward(
        ...
        encoder_outputs = self.encoder.exit_inference_forward(
            embedding_output,
            attention_mask=extended_attention_mask,
            head_mask=head_mask,
            encoder_hidden_states=encoder_hidden_states,
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.entropy_label = [None for _ in range(0, 12)]
    ...
    def exit_inference_forward(
        ...
        outputs, pooled_sequence_outputs = self.bert.exit_inference_forward(
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
