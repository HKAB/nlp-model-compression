...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  8 13:56:11 2021
...
#     return nn.CrossEntropyLoss()(output, targets.long())
...
def train(model, train_iter, val_iter, optimizer, scheduler, epochs):
    ...
    val_scores = []
    # best_score = None
    es = EarlyStopping(patience=5, path=RESULT_PATH + '/checkpoint.pt')
...
def main():
    ...
    parser.add_option('-l', '--layerdrop',
        ...
        help="Layerdrop rate. Default: 0.0", default=0.0)
    ...
    parser.add_option('-f', '--file',
        ...
        help="Pretrained file. Default: ''", default='')
    ...
    #     assert compute_metrics(predictions, targets)
        ...
        config = RobertaConfig(num_labels=num_classes, layerdrop=float(options.layerdrop))
        model = RobertaForTokenClassification.from_pretrained("vinai/phobert-base",
                                                              config=config)
        model.to(device)
