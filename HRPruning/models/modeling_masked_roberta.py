...
from .configuration_roberta import MaskedRobertaConfigForTokenClassification
...
_CONFIG_FOR_DOC = "MaskedRobertaConfigForTokenClassification"
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.query = MaskedLinear(
            ...
            head_split=config.num_attention_heads, # addition param
            bias_mask=True,
        ...
        # TODO: find a new way to mask QKV
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = self.transpose_for_scores(self.key(encoder_hidden_states, threshold=threshold))
            value_layer = self.transpose_for_scores(self.value(encoder_hidden_states, threshold=threshold))
            attention_mask = encoder_attention_mask
...
class RobertaSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, threshold=None, res_mask=None):
...
class RobertaAttention(nn.Module):
    ...
    def make_structural_pruning(self, blocksize):
        self.self.query.make_inference_pruning()
        self.self.key.make_inference_pruning()
        value_mask = self.self.value.make_inference_pruning()
        
        num_head = (value_mask.sum() // 64).item() # TODO: check 64
        self.self.num_attention_heads = int(num_head)
        self.self.all_head_size = self.self.attention_head_size * self.self.num_attention_heads
        
        self.output_mask = self.output.dense.make_inference_pruning()
        self.output.dense.make_column_pruning(value_mask)
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states, threshold,
                                       self.output_mask)
...
class RobertaLayer(nn.Module):
    ...
    def make_structural_pruning(self):
        self.attention.make_inference_pruning()
        intermediate_mask = self.intermediate.make_inference_pruning()
        self.output_mask = self.output.dense.make_inference_pruning()
        self.output.dense.make_column_pruning(intermediate_mask)
    ...
    def forward(
        ...
        layer_output = apply_chunking_to_forward(
            feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
        )
...
class RobertaPreTrainedModel(PreTrainedModel):
    ...
    config_class = MaskedRobertaConfigForTokenClassification
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def _make_structural_pruning(self, blocksize):
        for layer in range(len(self.bert.encoder.layer)):
            self.bert.encoder.layer[layer].make_structural_pruning()
...
class MaskedRobertaForTokenClassification(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
